...
def run_for_item_recipe(item, recipe, horadric_engine: HoradricEngine = None, explicit_ingredient_ids=None, min_avg_level=0, max_avg_level=100, print_summary=False, print_distribution=False, print_feasible_levels=False) -> None:
	...
	target_item_level = horadric_engine.item_db.items[item].required_wave_level
	...
	if print_summary:
		...
		for item_id in explicit_ingredient_ids:
			print(f"Item ID: {item_id}, with level {horadric_engine.item_db.items[item_id].required_wave_level}")
		print("-" * 30)
	...
	if print_feasible_levels:
		...
		for level, prob in curve.items():
			if prob != 0:
				print("-" * 30)
				print(f"Level: {level}, probability for item {item} with level {target_item_level}: {prob*100:.2f}%")
				if print_distribution:
					...
					for item_id, prob in avg_level_dist:
						print(f"Item ID: {item_id}, with level {horadric_engine.item_db.items[item_id].required_wave_level} probability: {prob*100:.2f}%")
					print("+" * 30)
